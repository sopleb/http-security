...
echo '<h2 class="nav-tab-wrapper"><a href="?page=http-security&tab=general-options" class="nav-tab">'. __('General options', 'http-security' ).'</a> <a href="?page=http-security&tab=csp-options" class="nav-tab">'. __('CSP options', 'http-security' ).'</a> <a href="?page=http-security&tab=permissions-policy" class="nav-tab">'. __('Permissions Policy', 'http-security' ).'</a> <a href="?page=http-security&tab=htaccess" class="nav-tab nav-tab-active">'. __('.htaccess (beta)', 'http-security' ).'</a></h2>';
...
if ( get_option( 'http_security_permissions_policy_flag' ) ) {
	$header_string = 'Permissions-Policy:';
	if ( get_option( 'http_security_permissions_policy_autoplay' ) ) $header_string .= ' autoplay '. esc_html( get_option( 'http_security_permissions_policy_autoplay' ) ).';';
	if ( get_option( 'http_security_permissions_policy_camera' ) ) $header_string .= ' camera '. esc_html( get_option( 'http_security_permissions_policy_camera' ) ).';';
	if ( get_option( 'http_security_permissions_policy_document_domain' ) ) $header_string .= ' document-domain '. esc_html( get_option( 'http_security_permissions_policy_document_domain' ) ).';';
	if ( get_option( 'http_security_permissions_policy_encrypted_media' ) ) $header_string .= ' encrypted-media '. esc_html( get_option( 'http_security_permissions_policy_encrypted_media' ) ).';';
	if ( get_option( 'http_security_permissions_policy_fullscreen' ) ) $header_string .= ' fullscreen '. esc_html( get_option( 'http_security_permissions_policy_fullscreen' ) ).';';
	if ( get_option( 'http_security_permissions_policy_geolocation' ) ) $header_string .= ' geolocation '. esc_html( get_option( 'http_security_permissions_policy_geolocation' ) ).';';
	if ( get_option( 'http_security_permissions_policy_microphone' ) ) $header_string .= ' microphone '. esc_html( get_option( 'http_security_permissions_policy_microphone' ) ).';';
	if ( get_option( 'http_security_permissions_policy_midi' ) ) $header_string .= ' midi '. esc_html( get_option( 'http_security_permissions_policy_midi' ) ).';';
	if ( get_option( 'http_security_permissions_policy_payment' ) ) $header_string .= ' payment '. esc_html( get_option( 'http_security_permissions_policy_payment' ) ).';';
	if ( get_option( 'http_security_permissions_policy_vr' ) ) $header_string .= ' vr '. esc_html( get_option( 'http_security_permissions_policy_vr' ) ).';';
	echo($header_string."\n");
}
